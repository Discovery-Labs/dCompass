...
contract PathwayNFT is ERC721URIStorage, ERC721Enumerable, Ownable{
    ...
    mapping (string => uint) public votes;//tally of approved votes per pathwayId;
    mapping (string => mapping(address => bool)) public reviewerVotes;//vote record of approved voters for PathwayId

    enum PathwayStatus{ NONEXISTENT, PENDING, DENIED, APPROVED }
    ...
    function voteForApproval(address[] memory _contributors, string memory _pathwayId, string memory _projectId, bytes32[2] memory r, bytes32[2] memory s, uint8[2] memory v, uint votesNeeded) public {
        require(status[_pathwayId] != PathwayStatus.DENIED && status[_pathwayId] != PathwayStatus.APPROVED, "finalized badge");
        require(!reviewerVotes[_pathwayId][_msgSender()], "already voted for this badge");
        bool voteAllowed = verifyContract.metaDataVerify(_msgSender(), _pathwayId, _projectId, r[0], s[0], v[0]);
        require(voteAllowed, "sender is not approved project voter");
        bool thresholdCheck = verifyContract.thresholdVerify(_msgSender(), _pathwayId, votesNeeded, r[1], s[1], v[1]);
    ...
    function createToken(string memory _tokenURI, string memory _pathwayId, string memory _projectId, bytes32[2] memory r, bytes32[2] memory s, uint8[2] memory v, uint votesNeeded) public returns(uint[] memory){
        require(!pathwayMinted[_pathwayId], "already minted");
        //require(vrfContract.blockNumberResults(_pathwayId) > 0, "no request yet");
        bool mintAllowed = verifyContract.metaDataVerify(_msgSender(), _pathwayId, _projectId, r[0], s[0], v[0]);
        ...
        require(status[_pathwayId] == PathwayStatus.APPROVED, "can only mint for pathways in approved status");        

        //batch minting
        uint256[] memory newItems = new uint256[](contributors[_pathwayId].length);
        uint256 newItemId;

        for(uint i =0; i< contributors[_pathwayId].length; i++){
        _tokenIds.increment();
        ...
        emit NFTPathwayMinted(contributors[_pathwayId][i], _tokenURI, _pathwayId);
        }
