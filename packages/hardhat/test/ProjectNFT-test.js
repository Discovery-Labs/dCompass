...
describe("ProjectNFT", function() {
  ...
  describe("Testing deployment and voting", function() {
    it("testing constructor and reviewer setters and getters", async function() {  
      ...
      //not a reviewer
      await expect(sponsorSFT.connect(addrs[0]).setStakeAmounts(1, `0xde0b6b3a7640000`)).to.be.revertedWith("not approved app reviewer");
      //id does not exist
      await expect(sponsorSFT.connect(addr3).setStakeAmounts(5, `0xde0b6b3a7640000`)).to.be.revertedWith("invalid _tokenId");
      ...
      for (let j=0; j< chainIds.length; j++){
        expect (await appDiamond.getERC20Addrs(chainIds[j])).to.be.eql(chainAddrObj[chainIds[j]]);
      }
    ...
    it("When threshold is 10 should approve right away", async function() {
      ...
      await expect(projectNFT.connect(addr1).projectRefund("firstTestProject", {value : "0x6F05B59D3B200000".toLowerCase()})).to.be.revertedWith("wrong sender");//not App wallet
      await expect(projectNFT.connect(owner).projectRefund("secondTestProject", {value : "0x6F05B59D3B200000".toLowerCase()})).to.be.revertedWith("incorrect status");//project does not exist
      ...
      await expect(projectNFT.connect(addr4).updateSponsorLevel("firstTestProject", "DIAMOND", {value: "0x1BC16D674EC80000".toLowerCase()})).to.be.revertedWith("insufficent funds for new level");
      ...
      await projectNFT.connect(addr4).updateSponsorLevel("firstTestProject", "GOLD", {value: "0x1BC16D674EC80000".toLowerCase()});
      ...
      await projectNFT.connect(addr4).updateSponsorLevel("firstTestProject", "DIAMOND", {value: "0x29A2241AF62C0000".toLowerCase()});
      ...
      await expect(sponsorSFT.connect(addrs[0]).setStakeAmounts(3, "0x3782DACE9D900000".toLowerCase())).to.be.revertedWith("not approved app reviewer");
      //changing diamond stake amount from 5 eth to 4 Eth
      await sponsorSFT.connect(addr3).setStakeAmounts(3, "0x3782DACE9D900000".toLowerCase());
      ...
      console.log(balance.toString());
      await projectNFT.connect(addr4).updateSponsorLevel("firstTestProject", "DIAMOND");
      stakeTest = await projectNFT.stakePerProject("firstTestProject");
    ...
    it("This should reject and send money to the project Wallet", async function() {
      ...
      await projectNFT.connect(addr6).addProjectWallet("firstTestProject", `${owner.address}`, "DIAMOND" , {value : `0x4563918244f40000`});
      balance = await ethers.provider.getBalance(addr6.address);
      console.log(`address 6 balance : ${balance.toString()}`);
      balance = await ethers.provider.getBalance(projectNFT.address);
      console.log(`nft contract balance : ${balance.toString()}`);
      await projectNFT.voteForApproval([`${addr5.address}`, `${addr6.address}`, "0xCf642913012CaBCBF09ca4f18748a430fA01237e", "0x4E7a45148C65248183AE1CE6C33fFAE5825C1979", "0x4553d50bEAf37ea51430C7E192ec4283d9B5BD56"], 25, "firstTestProject");
      ...
      expect(await projectNFT.projectMinted("firstTestProject")).to.be.false;//not minted yet
      expect(await projectNFT.projectThresholds("firstTestProject")).to.be.equal(25);//threshold for project set to 25

      //first rejection vote
      await projectNFT.connect(addr1).voteForRejection("firstTestProject");
      expect(await projectNFT.reviewerVotes("firstTestProject", `${addr1.address}`)).to.be.true;//reviewer who rejected is now true
      expect(await projectNFT.reviewerVotes("firstTestProject", `${addr2.address}`)).to.be.false;//reviewer who hasn't voted yet is false
      expect(await projectNFT.votes("firstTestProject")).to.be.equal(1);//1 vote cast for approval 
      ...
      await expect(projectNFT.connect(addr1).projectRefund("firstTestProject", {value : "0x6F05B59D3B200000".toLowerCase()})).to.be.revertedWith("wrong sender");//not App wallet
      await projectNFT.connect(owner).projectRefund("firstTestProject", {value : "0x6F05B59D3B200000".toLowerCase()});
      ...
      await projectNFT.connect(owner).updateSponsorLevel("firstTestProject", "DIAMOND");
      stakeTest = await projectNFT.stakePerProject("firstTestProject");
      expect(stakeTest.toString()).to.be.equal('4000000000000000000');
      balance = await ethers.provider.getBalance(owner.address);
      console.log(` owner balance after diamond refund : ${balance.toString()}`);
      
      //will get refund automatically since project went down to Silver
      await projectNFT.connect(owner).updateSponsorLevel("firstTestProject", "SILVER");
      expect(await projectNFT.sponsorLevel("firstTestProject")).to.be.equal(1);//silver sponsor level
      stakeTest = await projectNFT.stakePerProject("firstTestProject");
      expect(stakeTest.toString()).to.be.equal('1000000000000000000');
      balance = await ethers.provider.getBalance(owner.address);
      console.log(` owner balance after silver refund :  ${balance.toString()}`);


      //will get rejected unless sending enough value to update level to Gold
      await expect(projectNFT.connect(owner).updateSponsorLevel("firstTestProject", "GOLD", {value : "0xDE0B6B3A7640000".toLowerCase()})).to.be.revertedWith("insufficent funds for new level");

      //should be better not that sending 6 ETH (of which only 2 eth will go to contract and rest will be refunded)
      await projectNFT.connect(owner).updateSponsorLevel("firstTestProject", "GOLD", {value : "0x53444835EC580000".toLowerCase()});
      expect(await projectNFT.sponsorLevel("firstTestProject")).to.be.equal(2);//gold sponsor level
      stakeTest = await projectNFT.stakePerProject("firstTestProject");
      expect(stakeTest.toString()).to.be.equal('3000000000000000000');
      balance = await ethers.provider.getBalance(owner.address);
      ...
      expect(await projectNFT.reviewerVotes("firstTestProject", `${addr2.address}`)).to.be.true;//reviewer who rejected is now true
      expect(await projectNFT.votes("firstTestProject")).to.be.equal(1);//1 vote cast for approval 
      ...
      await expect(projectNFT.connect(owner).projectRefund("firstTestProject", {value : "0x6F05B59D3B200000".toLowerCase()})).to.be.revertedWith("incorrect status");//project has denied status
