...
const createModels = async () => {
  ...
  for (const [schemaName, schema] of Object.entries(schemas.dProfiles)) {
    ...
    if (createdSchemaID) {
      ...
      const definition = await manager.createDefinition(
        "@dProfiles/" + schemaName.toLowerCase(),
        {
          name: "@dProfiles/" + schemaName.toLowerCase(),
          description: `dProfiles schema definition for ${schemaName}.`,
          schema: schemaURL,
        }
      );
