...
import { NFTOwnerQuest } from '../NFTOwnerQuest.entity';
import { ForbiddenError } from 'apollo-server-express';

@Resolver(() => NFTOwnerQuest)
export class CreateNFTOwnerQuestResolver {
  @Mutation(() => NFTOwnerQuest, {
    nullable: true,
    description: 'Create a new NFT Owner Quest in dCompass',
    name: 'createNFTOwnerQuest',
  })
  async createNFTOwnerQuest(
    @UseCeramic() { ceramicClient }: UseCeramicClient,
    @Args('input') { id, questCreatorSignature }: CreateQuestInput,
  ): Promise<NFTOwnerQuest | null | undefined> {
    // Check that the current user is the owner of the quest
    const ogQuest = await ceramicClient.ceramic.loadStream(id);
    console.log({ ogQuestDoc: ogQuest.content });
