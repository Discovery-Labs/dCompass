...
import { GithubContributorQuest } from '../GithubContributorQuest.entity';
import { ForbiddenError } from 'apollo-server-express';

@Resolver(() => GithubContributorQuest)
export class CreateGithubContributorQuestResolver {
  @Mutation(() => GithubContributorQuest, {
    nullable: true,
    description: 'Create a new Github Contributor Quest in dCompass',
    name: 'createGithubContributorQuest',
  })
  async createGithubContributorQuest(
    @UseCeramic() { ceramicClient }: UseCeramicClient,
    @Args('input') { id, questCreatorSignature }: CreateQuestInput,
  ): Promise<GithubContributorQuest | null | undefined> {
